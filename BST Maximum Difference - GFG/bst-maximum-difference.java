...
class Solution
{
    ...
    {
        //Please code here
       return vishal(root,target,0);
    }
    public static int vishal(Node root,int target,int sum)
    {
        if(root==null)return -1;
        if(root.data<target)
        {
            return vishal(root.right,target,sum+root.data);
        }
        else if(root.data>target)
        {
            return vishal(root.left,target,sum+root.data);
        }
        else
        {
    ...
    public static int minimumSum(Node root)
    {
        ...
        if(root.left==null&&root.right==null)
        {
         ...
         else if(root.left==null)return minimumSum(root.right)+root.data;
        else if(root.right==null)return minimumSum(root.left)+root.data;
        else
